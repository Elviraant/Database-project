...
public class TestDatabase {
  ...
  public void testUniqueTableName() {
    Assert.assertTrue("Failure - Name of table is not unique", db.uniqueTableName(missingName));
    Assert.assertFalse("Failure - Name of table is not used by other Table", db.uniqueTableName(existingName));

  }

  @Test
  public void testCheckingCorrelation() {
    Assert.assertTrue("Failure - Correlation doesn't exist", db.checkingCorrelation(table1, table2));
    Assert.assertFalse("Failure - Correlation exists", db.checkingCorrelation(table1, table3));
